...
$prev_service_id = 0;
$next_service_id = 0;
$prev_rs = mysqli_query($con, "SELECT id FROM service WHERE id < '$todo' ORDER BY id DESC LIMIT 1");
if ($prev_rs) {
    $prev_row = mysqli_fetch_assoc($prev_rs);
    $prev_service_id = $prev_row ? (int)$prev_row['id'] : 0;
}
$next_rs = mysqli_query($con, "SELECT id FROM service WHERE id > '$todo' ORDER BY id ASC LIMIT 1");
if ($next_rs) {
    $next_row = mysqli_fetch_assoc($next_rs);
    $next_service_id = $next_row ? (int)$next_row['id'] : 0;
}
...
$service_media_items = [];
if ($has_service_media_table) {
    $m_rs = mysqli_query($con, "SELECT id, file_path, media_type FROM service_media WHERE service_id='$todo' ORDER BY id ASC");
    if ($m_rs) {
        while ($mrow = mysqli_fetch_assoc($m_rs)) {
            $service_media_items[] = $mrow;
        }
    }
}
?>
        ...
        <section class="section about-area ptb_100">
            <div class="container">
                <div class="row justify-content-between align-items-center">
                    ...
                    <h6 class="mb-3">Updated at: <?php print $updated_at?></h6>
                        <!-- About Thumb -->
                    ...
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="about-content section-heading text-center text-lg-left pl-md-4 mt-5 mt-lg-0 mb-0">
                            ...
                            <?php } ?>
                            <?php if ($prev_service_id > 0 || $next_service_id > 0) { ?>
                                <div class="d-flex justify-content-between align-items-center gap-2 mt-4">
                                    <div>
                                        <?php if ($prev_service_id > 0) { ?>
                                            <a class="btn btn-bordered" href="servicedetail.php?id=<?php print (int)$prev_service_id; ?>">Previous</a>
                                        <?php } ?>
                                    </div>
                                    <div>
                                        <?php if ($next_service_id > 0) { ?>
                                            <a class="btn btn-bordered active" href="servicedetail.php?id=<?php print (int)$next_service_id; ?>">Next</a>
                                        <?php } ?>
                                    </div>
                                </div>
                            <?php } ?>
                ...
                <?php if (!empty($service_media_items)) { ?>
                    <div class="row mt-5">
                        <div class="col-12">
                            <div class="media-carousel-section p-4">
                                <h3 class="mb-4 text-center">More Media</h3>
                                <div class="media-carousel owl-carousel owl-theme">
                                    <?php foreach ($service_media_items as $mitem) {
                                        ...
                                        $full_url = "dashboard/uploads/services/" . $mpath_safe;

                                        if ($mtype === 'image') {
                                            print "<div class='media-item'><a href='$full_url' data-fancybox='service-media' class='media-link'><div class='media-image-wrapper'><img src='$full_url' alt='media'></div><div class='media-overlay'><i class='fas fa-expand'></i></div></a></div>";
                                        } elseif ($mtype === 'video') {
                                            print "<div class='media-item'><a href='$full_url' data-fancybox='service-media' data-type='video' class='media-link'><div class='media-image-wrapper media-video-thumb'><div class='media-video-play'><i class='fas fa-play'></i></div></div></a></div>";
                                        } else {
                                            print "<div class='media-item'><a class='media-document' href='$full_url' data-fancybox='service-media' data-type='iframe' rel='noopener noreferrer'><i class='fas fa-file-alt fa-3x'></i><span>Open Document</span></a></div>";
                                        }
                                    } ?>
                                </div>
                            </div>
